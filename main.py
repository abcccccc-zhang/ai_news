...
"""
TrendRadar 主程序（main.py）

作用：
- 从配置（config/config.yaml）加载运行参数与平台列表
- 使用 DataFetcher 调用上游 newsnow 接口抓取各平台热点
- 将抓取结果规范化并保存到 output/日期/txt 与 html 目录
- 基于频率词（config/frequency_words.txt）进行统计聚合，最终生成多渠道推送与报表

与 MCP 的关系：
- MCP 服务器位于 mcp_server/server.py
- SystemManagementTools.trigger_crawl 提供“临时手动爬取”，逻辑与本文件 DataFetcher 基本一致，可选择是否持久化到 output

阅读顺序建议：
1) load_config：读取配置与通知渠道
2) DataFetcher：抓取核心（fetch_data / crawl_websites）
3) 数据处理：save_titles_to_file / parse_file_titles / read_all_today_titles
4) 统计分析：count_word_frequency / calculate_news_weight
5) 报告输出：generate_html_report / format_title_for_platform
"""
...
def load_config():
    """
    加载配置文件（config/config.yaml）并融合环境变量，生成统一的运行配置。

    主要内容：
    - app/version_check_url, show_version_update：版本检查开关
    - crawler/request_interval, enable_crawler, use_proxy, default_proxy：爬虫相关
    - report/mode, rank_threshold, sort_by_position_first, max_news_per_keyword：统计展示策略
    - notification/* 与 webhooks：推送渠道（飞书、钉钉、企业微信、Telegram、邮件、ntfy、Bark、Slack）
    - weight/*：热点权重计算（排名/频次/热度）
    - platforms：需要监控的平台列表（id/name）

    环境变量优先：
    - 例如 ENABLE_CRAWLER, ENABLE_NOTIFICATION 等，若设置则覆盖 config 文件值

    返回：
    - dict，包含下游模块直接使用的扁平化配置键，如 REQUEST_INTERVAL、PLATFORMS 等
    """
...
class DataFetcher:
    ...
    职责：
    - 单平台抓取：fetch_data() 负责根据平台 id 构造上游接口 URL 进行请求，并带有重试与随机等待
    - 多平台调度：crawl_websites() 负责遍历平台列表，逐个调用 fetch_data 并做节流；最后归并为统一结果结构

    说明：
    - 实际抓取依赖上游聚合服务（newsnow.busiyi.world），本项目不直接解析各站点 HTML
    - 该类是 main.py 的“爬虫”实现；MCP 侧也有一份临时抓取（SystemManagementTools.trigger_crawl），两者行为保持一致
    """
    ...
    def fetch_data(
        ...
        """
        抓取单个平台的热点数据（带重试与随机等待）

        参数：
        - id_info: 平台标识
            - str: 平台 id（等同别名）
            - Tuple[str, str]: (id, 别名/展示名称)
        - max_retries: 最大重试次数（默认 2）
        - min_retry_wait / max_retry_wait: 失败后随机等待区间（秒）

        请求：
        - URL: https://newsnow.busiyi.world/api/s?id={id}&latest
        - headers: 标准浏览器 UA、Accept 等
        - proxy: 若在 CONFIG 中开启代理，则走代理

        返回：
        - (data_text, id_value, alias)
          - data_text 为原始 JSON 字符串；解析失败或网络异常会返回 None
          - id_value 为平台 id；alias 为展示名（无则与 id 相同）
        - 若最终失败：返回 (None, id_value, alias)
        """
...
def _build_ai_prompt_from_report(report_data: Dict, mode: str = "daily") -> str:
    """
    生成供 AI 使用的提示文本：支持“topic（话题聚合）/news（最新新闻优先）/auto（自动）”。
    - AI_SUMMARY_MODE: auto|topic|news（默认 auto）
    - AI_TOPICS: 采样话题上限（默认 8）
    - AI_TITLES_PER_TOPIC: 每话题标题上限（默认 3）
    - AI_NEWS_TOPK: 新闻模式下的标题采样上限（默认 8）

    自动模式策略：当话题数≤2或总覆盖标题≤3或没有有效话题时，优先使用 news 模式，聚焦最新一条；否则用 topic 模式。
    """
    import os
    from datetime import datetime
    ...
    # 读取可调参数
    summary_mode = os.environ.get("AI_SUMMARY_MODE", "auto").strip().lower()
    # 若显式要求排除话题，则强制进入新闻模式
    if os.environ.get("AI_EXCLUDE_TOPICS", "false").lower() in ("1", "true", "yes"):
        summary_mode = "news"
    try:
        max_topics = int(os.environ.get("AI_TOPICS", "8"))
    except Exception:
        max_topics = 8
    try:
        titles_per_topic = int(os.environ.get("AI_TITLES_PER_TOPIC", "3"))
    except Exception:
        titles_per_topic = 3
    try:
        news_topk = int(os.environ.get("AI_NEWS_TOPK", "8"))
    except Exception:
        news_topk = 8

    total_topics = len(stats)
    total_titles_est = sum(s.get("count", 0) for s in stats)

    # 组装一个“候选新闻列表”（用于 news 模式或 auto 兜底）
    def collect_news_samples() -> List[Dict]:
        items: List[Dict] = []
        # 优先使用“新增新闻”作为最新样本
        for src in report_data.get("new_titles", []) or []:
            src_name = src.get("source_name")
            for t in src.get("titles", []) or []:
                items.append({
                    "title": t.get("title", ""),
                    "source": src_name or t.get("source_name", ""),
                    "ranks": t.get("ranks", []),
        ...
        uniq = []
        for it in items:
            key = (it.get("title"), it.get("source"))
            if key in seen:
                continue
            seen.add(key)
            uniq.append(it)
        return uniq[: max(news_topk, 1)]
    ...
    if summary_mode == "news":
        ...
        # auto
        if total_topics <= 2 or total_titles_est <= 3 or not stats:
            use_news_mode = True

    lines.append(f"数据模式: {mode}")

    if use_news_mode:
        # 新闻优先：聚焦最新一条 + 若干扩展
        news_items = collect_news_samples()
        lines.append(f"新闻模式：样本 {len(news_items)} 条（优先分析最新一条）")
        if news_items:
            latest = news_items[0]
            rks = latest.get("ranks", [])
            best_rank = min(rks) if rks else "-"
            lines.append("最新一条：")
            lines.append(f"- {latest.get('source','')}#{best_rank} | {latest.get('time_display','')} | {latest.get('title','')}")
            if len(news_items) > 1:
                lines.append("扩展观察（最近条目采样）：")
                for it in news_items[1:news_topk]:
                    r = it.get("ranks", [])
                    br = min(r) if r else "-"
                    lines.append(f"- {it.get('source','')}#{br} | {it.get('time_display','')} | {it.get('title','')}")
        else:
            lines.append("（暂无可用新闻样本）")
    else:
        # 话题聚合模式：保留原有话题风格，但限制采样规模
        lines.append(f"话题数: {total_topics} | 覆盖标题估算: {total_titles_est}")
        # 平台分布统计（基于采样标题）
        platform_counter: Dict[str, int] = {}
        for stat in stats[:max_topics]:
            for t in stat.get("titles", [])[:5]:
                src = t.get("source_name", "") or ""
                if src:
                    platform_counter[src] = platform_counter.get(src, 0) + 1
        if platform_counter:
            top_platforms = sorted(platform_counter.items(), key=lambda x: -x[1])[:6]
            plat_text = ", ".join([f"{k}:{v}" for k, v in top_platforms])
            lines.append(f"平台采样分布(Top): {plat_text}")

        lines.append("重点话题采样：")
        for i, stat in enumerate(stats[:max_topics], 1):
            word = stat.get("word", "")
            count = stat.get("count", 0)
            lines.append(f"[{i}] {word} | {count}条")
            for t in stat.get("titles", [])[:titles_per_topic]:
                title = t.get("title", "")
                source = t.get("source_name", "")
                ranks = t.get("ranks", [])
                best_rank = min(ranks) if ranks else "-"
                lines.append(f" - {source}#{best_rank}: {title}")

    # 失败平台（可选）
    if report_data.get("failed_ids") and os.environ.get("AI_INCLUDE_FAILED", "true").lower() in ("1","true","yes"):
        lines.append("失败平台: " + ", ".join(map(str, report_data["failed_ids"])) )

    return "\n".join(lines)


def _collect_news_items_from_report(report_data: Dict, news_topk: int) -> List[Dict]:
    """从 report_data 中提取新闻条目的 JSON（不依赖话题词）。优先取新增新闻，其次从所有标题中抽样。"""
    items: List[Dict] = []
    # 优先使用新增新闻
    for src in report_data.get("new_titles", []) or []:
        src_name = src.get("source_name")
        for t in src.get("titles", []) or []:
            items.append({
                ...
                "source": src_name or t.get("source_name", ""),
                "ranks": t.get("ranks", []),
                "time": t.get("time_display", ""),
                "url": t.get("url", "") or t.get("mobile_url", ""),
            })
    if not items:
        # 从所有统计标题里抽样（不读取话题词本身）
        for s in report_data.get("stats", []) or []:
            for t in s.get("titles", []) or []:
                items.append({
                    "title": t.get("title", ""),
                    "source": t.get("source_name", ""),
    ...
    for it in items:
        ...
        seen.add(key)
        uniq.append(it)
    # 排序：时间优先（无法解析时间则置后），排名次优
    def _rank_val(rks: List[int]):
        try:
            return min(rks) if rks else 999
        except Exception:
            return 999
    def _time_val(t: str):
        # 期望格式："HH时MM分" 或 "[HH时MM分 ~ HH时MM分]" 的起始
        try:
            s = t or ""
            m = re.search(r"(\d{1,2})时(\d{1,2})分", s)
            if not m:
                return -1
            ...
            return -1
    uniq.sort(key=lambda x: (-_time_val(x.get("time")), _rank_val(x.get("ranks", []))))
    return uniq[: max(news_topk, 1)]


def _is_finance_related_content(news_items: List[Dict]) -> bool:
    """
    检查新闻内容是否与金融相关。
    如果新闻中包含金融相关关键词，则认为与金融相关，可以进行 AI 分析。
    否则返回 False，跳过 AI 分析。
    ...
    - 金融、投资、交易、涨跌、行情等金融术语
    - 央行、证监会、交易所等金融机构
    - 经济、GDP、CPI、利率、汇率等经济指标
    """
    # 金融相关关键词库
    finance_keywords = {
        # 金融产品
        "股票", "基金", "债券", "期货", "期权", "ETF", "LOF", "理财", "信托", "ABS",
        # 市场
        "A股", "港股", "美股", "沪深", "创业板", "科创板", "新三板", "股市", "股价", "股指",
        # 金融术语
        "涨", "跌", "行情", "交易", "成交", "成交量", "换手", "振幅", "涨停", "跌停",
        "买入", "卖出", "开盘", "收盘", "高开", "低开", "跳空", "回调", "反弹",
        # 金融机构
        "央行", "证监会", "交易所", "银行", "券商", "基金公司", "期货公司", "保险",
        # 经济指标
        "GDP", "CPI", "PPI", "PMI", "失业率", "利率", "汇率", "汇兑", "外汇", "人民币",
        "美元", "欧元", "日元", "英镑", "港币", "新加坡元",
        # 投资相关
        "投资", "融资", "融券", "做空", "做多", "杠杆", "配资", "套利", "对冲", "风险",
        "收益", "回报", "分红", "派息", "除权", "除息", "送股", "转增",
        # 公司财务
        "财报", "业绩", "营收", "利润", "净利", "毛利", "EPS", "PE", "PB", "ROE",
        "资产", "负债", "现金流", "流动性", "偿债", "破产", "重组", "并购", "IPO",
        # 市场情绪
        "牛市", "熊市", "震荡", "盘整", "底部", "顶部", "支撑", "压力", "突破",
        # 监管相关
        "监管", "政策", "调控", "改革", "开放", "制裁", "贸易", "关税", "协议",
        # 行业相关（金融相关的行业）
        "金融", "保险", "房地产", "房价", "楼市", "地产", "开发商", "物业",
        "能源", "电力", "煤炭", "石油", "天然气", "新能源", "光伏", "风电",
        "科技", "芯片", "半导体", "5G", "6G", "AI", "人工智能", "云计算", "大数据",
        "互联网", "电商", "社交", "游戏", "视频", "直播",
        "医药", "生物", "制药", "医疗", "疫苗",
        "汽车", "新能源车", "电动车", "造车",
        "消费", "食品", "饮料", "服装", "家电", "家居",
        ...
        "农业", "种植", "养殖", "农产品",
        # 其他金融相关
        "上市", "退市", "停牌", "复牌", "增发", "减持", "举牌", "要约", "私有化",
        "破净", "破发", "新股", "打新", "中签", "申购",
        # 大宗商品（新增）
        "大宗商品", "有色金属", "黑色金属", "贵金属", "金属", "矿产",
        "铜", "铝", "锌", "铅", "镍", "锡", "铜价", "铝价", "锌价",
        "金", "银", "铂金", "钯金", "黄金", "白银", "金价", "银价",
        "铁矿石", "螺纹钢", "钢材", "钢铁", "焦炭", "焦煤", "动力煤",
        "原油", "石油", "天然气", "燃料油", "汽油", "柴油", "油价",
        "农产品", "大豆", "玉米", "小麦", "棉花", "白糖", "豆粕",
        # 行业板块（新增）
        "板块", "概念股", "龙头股", "白马股", "蓝筹股", "成长股", "价值股",
        "题材股", "热门股", "妖股", "庄股", "ST股",
        # 商品市场（新增）
        "期货市场", "现货市场", "商品价格", "原材料", "供应链",
        "产能", "产量", "库存", "需求", "供需", "供给",
        # 价格相关（新增）
        "价格", "涨价", "降价", "提价", "调价", "成本", "利润率", "毛利率",
        "价格上涨", "价格下跌", "价格波动", "价格走势",
    }
    
    # 反向关键词（非金融内容的标志）
    non_finance_keywords = {
        "娱乐", "明星", "电视", "综艺", "音乐", "歌手", "演员", "导演",
        "体育", "足球", "篮球", "排球", "乒乓", "羽毛", "网球", "高尔夫", "棋牌",
        "游戏", "电竞", "主播", "直播",  # 注：游戏/直播可能有金融属性，但这里作为非金融标志
        "旅游", "景点", "酒店", "美食", "餐饮",
        "教育", "学校", "大学", "考试", "培训",
        "健康", "医学", "疾病", "治疗", "药物",  # 注：医药股除外
        "天气", "气候", "自然灾害",
        "社会", "犯罪", "事故", "灾难",
        "文化", "艺术", "书籍", "文学",
    }
    ...
    # 判断逻辑：
    # 1. 如果金融关键词出现次数 > 0，则认为是金融相关
    # 2. 如果金融关键词出现次数 == 0 且非金融关键词出现次数 > 0，则认为是非金融
    # 3. 如果都没有，则默认认为是金融相关（保守策略，避免误杀）
    
    if finance_count > 0:
        print(f"AI 分析：检测到 {finance_count} 个金融关键词，将进行 AI 分析")
        return True
    elif non_finance_count > 0:
        print(f"AI 分析：检测到 {non_finance_count} 个非金融关键词，跳过 AI 分析")
        return False
    else:
        # 默认认为是金融相关（保守策略）
        print(f"AI 分析：未检测到明确的金融或非金融关键词，按金融相关处理")
        return True


def _generate_ai_summary_with_openai(report_data: Dict, mode: str = "daily", proxy_url: Optional[str] = None) -> Optional[str]:
    """
    使用 OpenAI 生成 10~15 行的中文要点摘要。通过环境变量读取密钥：
    - OPENAI_API_KEY: 必填
    - OPENAI_BASE_URL: 可选，默认 https://api.openai.com/v1
    - OPENAI_MODEL: 可选，默认 gpt-4o-mini（可改成 gpt-3.5-turbo 以提升兼容性）

    返回：摘要文本；若未配置或失败返回 None
    """
    ...
    # 预先构建代理，供正文抓取与API调用共用
    proxies = {"http": proxy_url, "https": proxy_url} if proxy_url else None

    # 选择输入格式：json|text（默认 json）
    input_format = os.environ.get("AI_INPUT_FORMAT", "json").lower().strip()

    # 收集新闻 JSON（与话题无关），受 AI_NEWS_TOPK 控制
    try:
        news_topk = int(os.environ.get("AI_NEWS_TOPK", "8"))
    except Exception:
        news_topk = 8
    news_items = _collect_news_items_from_report(report_data, news_topk)
    
    # 【新增】检查内容是否与金融相关，如果不相关则直接返回 None，不调用 AI API
    if not _is_finance_related_content(news_items):
        print("AI 分析：内容与金融无关，跳过 AI 分析")
        return None

    # 【优化】抓取 URL 正文（默认启用，可通过环境变量禁用）
    # 现在默认为 true，确保 AI 有充分的内容进行分析
    fetch_flag = os.environ.get("AI_FETCH_CONTENT", "true").lower() not in ("0","false","no")
    try:
        fetch_topk = int(os.environ.get("AI_FETCH_TOPK", "15"))
    except Exception:
        fetch_topk = 15
    try:
        fetch_timeout = float(os.environ.get("AI_FETCH_TIMEOUT", "10"))
    except Exception:
        fetch_timeout = 10.0
    try:
        fetch_max_chars = int(os.environ.get("AI_FETCH_MAX_CHARS", "3000"))
    except Exception:
        fetch_max_chars = 3000

    if fetch_flag and news_items:
        print(f"AI 分析：准备抓取页面内容，最多 {min(fetch_topk, len(news_items))} 条，超时 {fetch_timeout}s，最大字符数 {fetch_max_chars}")
        headers_fetch = {
        ...
        fetched_count = 0
        for it in news_items[:max(fetch_topk, 1)]:
            url = (it.get("url") or "").strip()
            if not url.startswith("http"):
                continue
            try:
                r = requests.get(url, headers=headers_fetch, proxies=proxies, timeout=fetch_timeout)
                r.raise_for_status()
                text = r.text or ""
                ...
                if "text/plain" in ctype and text:
                    ...
                    # HTML 提取纯文本（优先 BeautifulSoup，失败则正则）
                    try:
                ...
                if content_txt.strip():
                    ...
                    fetched_count += 1
                    print(f"AI 分析：成功抓取 {url[:60]}... ({len(content_txt)} 字符)")
                else:
                    print(f"AI 分析：页面内容为空 {url[:60]}...")
                    
            except requests.Timeout:
                print(f"AI 分析：抓取超时 {url[:60]}...")
            except requests.RequestException as e:
                print(f"AI 分析：网络错误 {url[:60]}... ({type(e).__name__})")
            except Exception as e:
                print(f"AI 分析：抓取失败 {url[:60]}... ({e})")
        
        print(f"AI 分析：共成功抓取 {fetched_count}/{min(fetch_topk, len(news_items))} 条页面内容")

    # 【优化】构建更灵活的用户提示词
    if input_format == "json":
        # 构建结构化的新闻数据
        news_summary = json.dumps(news_items, ensure_ascii=False, indent=2)
        user_content = (
            f"以下是 {mode} 模式的热点新闻数据（共 {len(news_items)} 条）：\n\n"
            f"{news_summary}\n\n"
            f"请基于上述新闻数据进行深度分析，提供你的专业见解。"
        )
    else:
        # 兼容：用文本提示（旧逻辑）
        user_content = _build_ai_prompt_from_report(report_data, mode)
    ...
    # 【优化】更灵活的系统提示词，给 AI 更多分析空间
    system_prompt = os.environ.get("AI_SYSTEM_PROMPT", "").strip()
    if not system_prompt:
        # 默认系统提示词（更灵活，给 AI 更多空间）
        system_prompt = (
    ...
    try:
        print(f"AI 分析：正在调用 {model} 进行分析...")
        resp = requests.post(f"{base_url}/chat/completions", headers=headers, json=body, timeout=60, proxies=proxies)
        
        if resp.status_code != 200:
            print(f"AI 分析失败，HTTP {resp.status_code}: {resp.text[:200]}")
            return None
        ...
        if "error" in data:
            ...
            print(f"AI 分析 API 错误: {error_msg}")
            return None
        ...
        # 【优化】不再强制裁剪，保留完整的 AI 分析结果
        # 只在必要时进行行数限制（防止过长）
        max_lines = int(os.environ.get("AI_MAX_LINES", "50"))
        lines = [ln.strip() for ln in content.splitlines() if ln.strip()]
        
        if len(lines) > max_lines:
            print(f"AI 分析：结果行数 {len(lines)} 超过限制 {max_lines}，进行裁剪")
            result = "\n".join(lines[:max_lines])
        else:
            result = "\n".join(lines)
        
        print(f"AI 分析：成功生成分析结果（{len(result)} 字符，{len(lines)} 行）")
        return result
        
    except requests.Timeout:
        print(f"AI 分析：请求超时（60秒）")
        return None
    except requests.RequestException as e:
        print(f"AI 分析：网络错误 ({type(e).__name__}): {e}")
        return None
    except Exception as e:
        print(f"AI 分析：异常错误: {e}")
        return None


def _send_feishu_text(webhook_url: str, text: str, proxy_url: Optional[str] = None) -> bool:
    """发送单条纯文本到飞书。"""
    ...
    try:
        r = requests.post(webhook_url, headers=headers, json=payload, timeout=30, proxies=proxies)
        if r.status_code == 200:
            res = r.json()
            return bool(res.get("StatusCode") == 0 or res.get("code") == 0)
        return False
    except Exception as e:
        print(f"发送飞书 AI 摘要失败: {e}")
        return False
...
def send_to_feishu(
    ...
    """发送到飞书（支持分批发送）。在正式内容前，若配置了 OPENAI_API_KEY，则先推送一条“🤖 AI 分析摘要”。"""
    headers = {"Content-Type": "application/json"}
    proxies = None
    if proxy_url:
        proxies = {"http": proxy_url, "https": proxy_url}

    # 先尝试生成 AI 摘要并单独推送一条（优先用 LangGraph Agent，失败回退到直连）
    ai_summary = None
    is_finance_related = True  # 默认认为是金融相关（保守策略）
    
    try:
        AGENT_ENABLED = os.environ.get("AI_AGENT_ENABLED", "true").lower() in ("1", "true", "yes")
    except Exception:
        AGENT_ENABLED = True
    
    # 是否启用推送前的金融相关性检查（默认启用）
    try:
        PUSH_FINANCE_CHECK_ENABLED = os.environ.get("PUSH_FINANCE_CHECK_ENABLED", "true").lower() in ("1", "true", "yes")
    except Exception:
        PUSH_FINANCE_CHECK_ENABLED = True
    
    if AGENT_ENABLED:
        try:
            from ai_agent import TrendRadarAgent
            from utils_ai import fetch_content, summarize, relevance_llm
            agent = TrendRadarAgent(
                fetch_content_fn=fetch_content,
                summarize_fn=summarize,
                collect_fn=_collect_news_items_from_report,
                relevance_fn=_is_finance_related_content,
                relevance_llm_fn=relevance_llm,
            )
            agent_result = agent.run(report_data, mode=mode, proxy_url=proxy_url)
            
            # 获取判断结果
            is_finance_related = agent_result.get("is_finance", True)
            ai_summary = agent_result.get("ai_summary")
            
            # 打印判断详情
            print(f"飞书推送：AI判断结果 - 金融相关: {is_finance_related} (规则: {agent_result.get('is_finance_rule')}, LLM: {agent_result.get('is_finance_llm')}) [{report_type}]")
            
            # 如果启用了金融检查且判断为非金融相关，则跳过推送
            if PUSH_FINANCE_CHECK_ENABLED and not is_finance_related:
                print(f"飞书推送：AI判断为非金融相关新闻，跳过推送 [{report_type}]")
                return True  # 返回True表示"正常跳过"，不是错误
                
        except Exception as e:
            print(f"AI Agent 运行失败，回退到直连: {e}")
            ai_summary = _generate_ai_summary_with_openai(report_data, mode=mode, proxy_url=proxy_url)
            # 回退模式下，如果没有生成摘要，可能是非金融相关
            if PUSH_FINANCE_CHECK_ENABLED and ai_summary is None:
                print(f"飞书推送：回退模式判断为非金融相关新闻，跳过推送 [{report_type}]")
                return True
    else:
        ai_summary = _generate_ai_summary_with_openai(report_data, mode=mode, proxy_url=proxy_url)
        # 非Agent模式下，如果没有生成摘要，可能是非金融相关
        if PUSH_FINANCE_CHECK_ENABLED and ai_summary is None:
            print(f"飞书推送：判断为非金融相关新闻，跳过推送 [{report_type}]")
            return True

    if ai_summary:
        preface = f"🤖 AI 分析摘要\n\n{ai_summary}\n\n{CONFIG.get('FEISHU_MESSAGE_SEPARATOR', '━━━━━━━━━━━━━━━━━━━')}\n"
        ok = _send_feishu_text(webhook_url, preface, proxy_url)
        print(f"飞书 AI 摘要发送结果: {ok}")
        if ok:
            # 与后续批次之间稍作间隔
            time.sleep(CONFIG.get("BATCH_SEND_INTERVAL", 3))
